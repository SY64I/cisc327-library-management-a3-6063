...
def update_borrow_record_return_date(patron_id: str, book_id: int, return_date: datetime) -> bool:
    ...
    try:
        ...
        conn.close()
        return False
# Custom function that fetches a patron's full borrow record
def get_patron_full_borrow_record(patron_id: str) -> List[Dict]:
    """Get currently borrowed books for a patron."""
    conn = get_db_connection()
    records = conn.execute('''
        SELECT br.*, b.title, b.author 
...
# Custom functon to clear all books and borrow records from the database for testing (almost resets database to default).
def clear_all_data() -> bool:
    """Clears the book catalog"""
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        #cur.execute('UPDATE SQLITE_SEQUENCE SET SEQ=0 WHERE NAME=books')
...
# Custom function to get book id from a known book isbn for testing purposes.
def get_book_id_by_isbn(isbn: str) -> int:
    """Get a specific book by ISBN."""
    conn = get_db_connection()
    book = conn.execute('SELECT * FROM books WHERE isbn = ?', (isbn,)).fetchone()
    book_id = conn.execute('''
        SELECT id as book_id FROM books
        WHERE isbn = ?
    ''', (isbn,)).fetchone()
    conn.close()
    return book['id']

# Custom function that manually adds an overdue book to a patron's records for testing purposes.
def borrow_test_late_book(patron_id: str, isbn: str, days_late: int) -> bool:
    """Add sample data to the database if it's empty."""
    conn = get_db_connection()
    try:
        # Add a test book into the system (specific isbn to not cause other issues)
        book = conn.execute('''
            INSERT INTO books (title, author, isbn, total_copies, available_copies)
            VALUES (?, ?, ?, ?, ?)
        ''', ('The Late Tests', 'G. Tester', isbn, 5, 4))
        ...
        # Add a borrow record for the specifed patron and make their return date late by specified days
        conn = get_db_connection()
        conn.execute('''
            INSERT INTO borrow_records (patron_id, book_id, borrow_date, due_date)
            VALUES (?, ?, ?, ?)
        ''', (patron_id, book_id, 
                (datetime.now() - timedelta(days=14 + days_late)).isoformat(),
                (datetime.now() - timedelta(days=days_late)).isoformat()))
